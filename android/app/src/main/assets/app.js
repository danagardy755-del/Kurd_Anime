...
const DEVICE_ID = localStorage.deviceId || (()=>{const v='android-'+makeId();localStorage.deviceId=v;return v;})();

const T = {
  ku:{home:'ماڵەوە',anime:'ئەنیمەکان',movies:'فیلم و زنجیرە',library:'کتێبخانە',more:'زیاتر',trending:'بەناوبانگ',latest:'نوێترین',search:'گەڕان',settings:'ڕێکخستنەکان',watch:'بینین',all:'هەموو',language:'زمان',query:'ناوی ئەنیمە بگەڕێ...',server:'بەستەری سێرڤەر',save:'پاشەکەوتکردن',loading:'چاوەڕوان بە...',offline:'پەیوەندی بە سێرڤەرەکە نییە.',noResults:'هیچ ئەنجامێک نەدۆزرایەوە',serverMissing:'سێرڤەری Kurd Anime هێشتا دانەنراوە.'},
  ...
  ar:{home:'الرئيسية',anime:'الأنمي',movies:'الأفلام والمسلسلات',library:'المكتبة',more:'المزيد',trending:'الأكثر رواجاً',latest:'الأحدث',search:'بحث',settings:'الإعدادات',language:'اللغة',watch:'مشاهدة',query:'ابحث عن أنمي...',server:'رابط الخادم',save:'حفظ',loading:'جار التحميل...',offline:'لا يمكن الوصول إلى خادم Kurd Anime.',noResults:'لم يتم العثور على نتائج',serverMissing:'لم يتم إعداد خادم Kurd Anime بعد.'}
};
const t = k => T[lang][k] || k;
async function api(path,opt={}) {
  if(!API) throw new Error('API_NOT_CONFIGURED');
  const controller = new AbortController(); const timer=setTimeout(()=>controller.abort(),10000);
  try { const r=await fetch(API.replace(/\/$/,'')+path,{...opt,signal:controller.signal,headers:{accept:'application/json','x-device-id':DEVICE_ID,...(opt.headers||{})}}); if(!r.ok) throw new Error('HTTP '+r.status); return await r.json(); }
  finally { clearTimeout(timer); }
}
function esc(s){return String(s??'').replace(/[&<>\"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[m]));}
function clean(s){return String(s??'').replace(/<[^>]*>/g,'').trim();}
function top(){return `<header class="top"><div class="brand">KURD<span>ANIME</span></div></header>`;}
function nav(){return `<nav class="bottom">${[['home','⌂'],['anime','◉'],['movies','▣'],['library','▤'],['more','☷']].map(x=>`<button class="nav ${page===x[0]?'active':''}" onclick="go('${x[0]}')">${x[1]}<small>${t(x[0])}</small></button>`).join('')}</nav>`;}
function state(msg){return `<div class="offline">ℹ️ ${esc(msg)}</div>`;}
function card(x){return `<div class="tile" onclick="detail('${esc(x.id)}')">${x.poster?`<img class="poster-img" src="${esc(x.poster)}" loading="lazy" alt="">`:`<div class="poster">${esc(x.title)}</div>`}<small>${esc(x.title)}</small><br><small>⭐ ${x.score??'-'}</small></div>`;}
...
async function home(){try{const j=await api('/anime/trending');return `${top()}<section class="hero"><h1>Kurd Anime</h1><p>ئەنیمەی ڕاستەقینە و داتای ڕاستەوخۆ</p></section><section class="section"><h2>${t('trending')}</h2>${list(j?.data||[])}</section>${nav()}`}catch(e){return `${top()}${state(t('offline'))}<section class="section"><h2>${t('trending')}</h2><p>${esc(e.message)}</p><button class="cta" onclick="render()">Retry</button></section>${nav()}`;}}
async function anime(){let items=[];try{items=(await api('/anime/trending'))?.data||[];}catch(e){}return `${top()}<section class="section"><h2>${t('anime')}</h2><input class="search" placeholder="${t('query')}" onkeydown="if(event.key==='Enter')search(this.value)">${items.length?list(items):state(t('offline'))}</section>${nav()}`;}
...
async function detail(id){try{const x=(await api('/anime/'+encodeURIComponent(id)))?.data;if(!x)throw Error('not found');document.getElementById('app').innerHTML=`${top()}<section class="hero">${x.banner?`<img class="banner" src="${esc(x.banner)}" alt="">`:''}<h1>${esc(x.title)}</h1><p>${x.year||''}${x.score!=null?' • ⭐ '+x.score:''}</p><button class="cta" onclick="episodes('${esc(x.id)}')">${t('watch')}</button></section><section class="section"><p>${esc(clean(x.description).slice(0,1600))}</p></section>${nav()}`;}catch(e){document.getElementById('app').innerHTML=`${top()}${state(t('offline'))}${nav()}`;}}
...
async function library(){try{const j=await api('/library/device');return `${top()}<section class="section"><h2>${t('library')}</h2>${list(j?.data?.favorites||[])}</section>${nav()}`}catch(e){return `${top()}${state(t('offline'))}${nav()}`;}}
function serverForm(){return `<section class="section"><h2>${t('settings')}</h2><label>${t('server')}</label><input id="server" class="search" value="${esc(API)}"><button class="cta" onclick="saveServer()">${t('save')}</button></section>`;}
...
async function movies(){return `${top()}<section class="section"><h2>${t('movies')}</h2><p>لە وەشانی داهاتوودا زیاد دەکرێت.</p></section>${nav()}`;}
