...
let page = 'home';
const DEVICE_ID = localStorage.deviceId || (()=>{const v='android-'+crypto.randomUUID();localStorage.deviceId=v;return v;})();
...
async function api(path,opt={}) {
  ...
  try {
    const r = await fetch(API.replace(/\/$/,'') + path,{...opt,signal:controller.signal,headers:{'accept':'application/json','x-device-id':DEVICE_ID,...(opt.headers||{})}});
...
async function home(){if(!API)return `${top()}<section class="hero"><h1>Kurd Anime</h1><p>${t('serverMissing')}</p></section>${serverForm()}${nav()}`;try{const j=await api('/anime/trending');return `${top()}<section class="hero"><h1>Kurd Anime</h1><p>${lang==='ku'?'ئەنیمەی ڕاستەقینە و داتای ڕاستەوخۆ':'Live anime metadata from the Kurd Anime API'}</p></section><section class="section"><h2>${t('trending')}</h2>${list(j?.data||[])}</section>${nav()}`}catch(e){return `${top()}${state(t('offline'))}<section class="section"><h2>${t('trending')}</h2><p>${esc(e.message)}</p></section>${nav()}`;}}
async function anime(){let items=[];try{items=(await api('/anime/trending'))?.data||[];}catch(e){}return `${top()}<section class="section"><h2>${t('anime')}</h2><input class="search" placeholder="${t('query')}" onkeydown="if(event.key==='Enter')search(this.value)">${!API?state(t('serverMissing')):items.length?list(items):state(t('offline'))}</section>${nav()}`;}
