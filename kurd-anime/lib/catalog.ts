...
/**
 * Stream URLs are intentionally not invented here.
 * Add only URLs supplied by the owner/licensor or an authorized API.
 */
export const makeEpisode = (number: number): Episode => ({
  number,
  title: `ئەڵقەی ${number}`,
  duration: "24 خولەک",
  sources: [],
...
const episodes = (count: number) =>
  Array.from({ length: count }, (_, i) => makeEpisode(i + 1));
...
export const STREAM_API_URL = process.env.EXPO_PUBLIC_STREAM_API_URL ?? "";
...
export function getSubtitles(episode: Episode): Subtitle[] {
  return episode.sources.flatMap((source) => source.subtitles ?? []).filter((subtitle) => Boolean(subtitle.url));
}
